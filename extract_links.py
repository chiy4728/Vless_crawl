...
def extract_proxy_links(url):
    """
    从给定的URL中提取 VLESS, VMess (包含 URL Scheme 和 Raw Base64), Hysteria2 和 AnyTLS 链接，
    进行高级去重，然后生成一个订阅链接。
    """
    try:
        headers = {
        ...
        anytls_links = re.findall(r'anytls://[a-zA-Z0-9\-\._~:/?#\[\]@!$&\'()*+,;=%]+', content)

        # 2. 提取不带协议头的 Raw Base64 VMess 节点
        ...
        # 合并所有链接（包含 AnyTLS）
        all_raw_links = vless_links + vmess_links + hysteria2_links + anytls_links + converted_vmess_links
        ...
        anytls_c = sum(1 for l in unique_links if l.startswith('anytls'))
        print(f"(VLESS: {vless_c}, VMess: {vmess_c}, Hysteria2: {hy2_c}, AnyTLS: {anytls_c})")
